...
export const PolizeipraesidienTile: React.FC<PolizeipraesidienTileProps> = ({
  ...
  const [InteractiveMapComponent, setInteractiveMapComponent] =
  ...
  const getRelevantSegments = useCallback(
    (
      ...
      stationsLower.forEach((station) => {
        ...
        Object.entries(ppCodeToCity).forEach(([code, city]) => {
          if (
            station.includes(city) ||
            station.includes(`pp ${code}`) ||
            station.includes(`polizeipräsidium ${city}`)
          ) {
        ...
        segments.forEach((segment) => {
          ...
          if (
            station.includes(segmentLabelLower) ||
            segmentLabelLower.includes(station)
          ) {
      ...
      return segments.filter((segment) => {
        ...
        const cityName =
          ppCode && ppCode in ppCodeToCity ? ppCodeToCity[ppCode] : "";
        const segmentLabelLower = segment.label.toLowerCase();

        // Prüfe ob Segment zu einer relevanten Stadt gehört
        const matchesCity = cityName ? relevantCities.has(cityName) : false;
        const matchesLabel = Array.from(relevantCities).some(
          (city) =>
            segmentLabelLower.includes(city) || city.includes(segmentLabelLower)
        );
  ...
  useEffect(() => {
    void import("@/components/map/PolizeiSVGComplete")
      .then((m) => ({ segments: m.segments }))
      .then((MapData) => {
  ...
  useEffect(() => {
    ...
    if (shouldLoadMap) {
      // Lade Karte
      void import("@/components/map/PolizeiSVGComplete")
        .then((m) => ({ Complete: m.PolizeiSVGComplete }))
  ...
  const renderListContent = (showKarteButton = false) => {
    ...
    const filteredPPs = Object.entries(groupedByPP).filter(
      ([ppId, segments]) => {
    ...
    return (
      <MapFilterProvider>
        ...
        >
          ...
          <div
            className="flip-container relative w-full flex-1 min-h-0"
            style={{ minHeight: "45vh" }}
          >
            {!isFlipped ? (
              /* Vorderseite: Liste */
              ...
              >
                <div className="h-full flex flex-col">
                  ...
                  <div
                    className="flex-1 bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 overflow-y-auto overflow-x-hidden relative w-full"
                    style={{
        ...
        <MapFilterProvider>
          ...
          <div className="hidden md:block absolute z-[60] mt-1 right-0 rounded-lg border border-border bg-white dark:bg-slate-800 shadow-2xl overflow-hidden">
            <div className={`flex flex-row ${showMap ? "" : "flex-row"}`}>
              ...
              {showMap && (
                <div
                  className="bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 border-r border-border"
                  style={{
                    width: "330px",
                    height: "372px",
              ...
              <div
                className={`flex-shrink-0 bg-white dark:bg-slate-800 flex flex-col ${showMap ? "" : "border-l-0"}`}
                style={{
                    ...
                    const filteredPPs = Object.entries(groupedByPP).filter(
                      ([ppId, segments]) => {
                        ...
                        const ppName = ppId
                          ...
                          .toUpperCase();
                        return (
                          ppName.toLowerCase().includes(term) ||
                          segments.some(
                            (s) =>
                              s.label.toLowerCase().includes(term) ||
                              s.id.toLowerCase().includes(term)
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <button
                                  onClick={(e) =>
                                    handlePPAccordionToggle(ppId, e)
                                  }
          ...
          >
            <div className="absolute bottom-0 left-0 right-0 max-h-[90vh] rounded-t-2xl bg-white dark:bg-slate-800 shadow-2xl overflow-hidden">
              {/* Flip Container für mobile Ansicht */}
              <div
                className="flip-container relative h-full"
                style={{ height: "90vh", perspective: "1200px" }}
              >
                <div
                    ...
                    <div className="flex flex-col h-full p-4">
                      ...
                      >
                        {(() => {
                          ...
                          // Filtere PP basierend auf Suchbegriff
                          const filteredPPs = Object.entries(
                            groupedByPP
                          ).filter(([ppId, segments]) => {
                            if (!searchTerm.trim()) return true;
                            const term = searchTerm.toLowerCase();
                            const ppName = ppId
                              .replace(/^pp-/, "PP ")
