...
export default function FahndungFilter({ onFilterChange, className = "" }: FahndungFilterProps) {
  ...
  const toggleFilter = (
    ...
    void setActiveFilters(newFilters);
  ...
  const activeFilterCount =
    [...activeFilters.status, ...activeFilters.priority, ...activeFilters.category].length +
    ...
    <div className={`fahndung-filter ${className}`}>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
        ...
        <div className="relative max-w-md flex-1">
          <label htmlFor="fahndung-filter-search" className="sr-only">
            Fahndungen durchsuchen
          </label>
          <input
            id="fahndung-filter-search"
            type="text"
                ...
                {filterOptions.timeRange.map((option) => (
                  <label key={option.value} className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={() => setTimeFilter(option.value as FilterState["timeRange"])}
                      className="rounded"
                    />
                    <span className="text-sm">{option.label}</span>
                  </label>
            ...
            <div className="mt-4 border-t pt-4">
              <div className="flex flex-wrap gap-2">
                {activeFilters.status.map((status) => (
                  ...
                  >
                    Status: {filterOptions.status.find((s) => s.value === status)?.label}
                  </span>
                ))}
                {activeFilters.priority.map((priority) => (
                  ...
                  >
                    {filterOptions.priority.find((p) => p.value === priority)?.label}
                  </span>
                ))}
                {activeFilters.category.map((category) => (
                  ...
                  >
                    {filterOptions.category.find((c) => c.value === category)?.label}
                  </span>
                ))}
                {activeFilters.timeRange !== "all" && (
                  <span className="inline-flex items-center rounded-full bg-purple-100 px-2 py-1 text-xs text-purple-800 dark:bg-purple-900 dark:text-purple-200">
                    {
                      filterOptions.timeRange.find((t) => t.value === activeFilters.timeRange)
                        ?.label
                    }
                  </span>
                )}
